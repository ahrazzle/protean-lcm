...
protean-lcm is a member of the Protean product family. It draws from the LCM paper concept (Ehrlich & Blackman, Voltropy PBC) and the design of stephenschoettler/hermes-lcm. We pulled the DAG-based compaction idea, the plugin-local SQLite store with FTS metadata, and the recall tools with recall-policy skill.
